...
const faqs = [{"q": "ブーツのぶかぶかはインソールで直せますか？", "a": "やや厚みのあるインソールを入れると容積が埋まり、フィット感が高まります。サイズが大きすぎる場合は厚みのある中敷きで調整しましょう。"}, {"q": "冬のブーツの底冷え対策になりますか？", "a": "保温性のある素材のインソールは、地面からの冷気を遮って足先の冷えを和らげる助けになります。"}, {"q": "ヒールのあるブーツでも使えますか？", "a": "はい。ヒールブーツは前足部に負担が集中するため、アーチサポートと前すべり防止のあるインソールが快適です。"}, {"q": "インソールを入れたらブーツがきつくなりました。どうすれば？", "a": "ブーツはもともと中の空間が狭めの製品が多く、厚いインソールを足すと窮屈になることがあります。まずは元々入っている中敷きを取り出してから入れ替える、より薄型のインソールを選ぶ、つま先側を少しカットするなどで圧迫を和らげられます。指先がしびれる・血色が悪くなる場合は使用を中止してください。"}, {"q": "インソールはブーツのサイズに合わせてカットできますか？", "a": "多くのインソールはカットラインの目安が印刷されており、ハサミで余分な部分を切って調整できます。最初は大きめに切り、実際にブーツへ入れて様子を見ながら少しずつ詰めると失敗しにくくなります。製品によってはカット不可のものもあるため、各商品の表記を確認してください。"}];
    ...
    <>
      ...
      <article className="max-w-3xl mx-auto px-4 py-8 md:py-12">
        ...
        <section className="mb-12">
          ...
          <p className="text-sm md:text-base text-gray-700 leading-relaxed mb-4">ロングブーツは丈が長く通気しにくいぶん内部に湿気と熱がこもりやすく、ニオイの原因になりがちです。脱いだ後にインソールを取り出して陰干しし、消臭・抗菌素材のものを選ぶと衛生的に使えます。同じインソールを複数用意して日替わりで乾かしながら使うのも効果的です。</p>
          <p className="text-sm md:text-base text-gray-700 leading-relaxed mb-4">ヒールの高いブーツでは足が前方へ滑り、つま先が詰まって指が痛くなりやすいのが悩みです。土踏まずを支えるアーチサポートや、前足部・かかとのホールド感があるインソールを選ぶと、足が前に流れにくくなり指への圧迫を抑えられます。</p>
          <p className="text-sm md:text-base text-gray-700 leading-relaxed mb-4">注意したいのは厚みの足しすぎです。ブーツは中の空間が狭めの製品が多く、厚いインソールを入れると甲やつま先が窮屈になることがあります。元々の中敷きを抜いてから入れ替える、薄型を選ぶ、つま先側をカットして調整するなど、フィット感を見ながら調整しましょう。</p>
        </section>

...
const faqs = [{"q": "野球スパイクにインソールは入りますか？", "a": "薄型〜標準のインソールなら入ります。スパイクは内寸が決まっているため、元の中敷きを外して入れ替えるのがおすすめです。"}, {"q": "走塁や打撃に効果はありますか？", "a": "アーチを支えると踏み込みの力が伝わりやすくなり、安定したダッシュやスイングの土台になります。劇的ではありませんが足元の安定感が高まります。"}, {"q": "試合後半の足の疲れも軽減できますか？", "a": "クッションとアーチサポートのないスパイクの弱点を補い、長時間の試合・練習での足裏の疲れを軽減します。"}, {"q": "金属スパイクとポイント（樹脂）スパイクで選び方は変わりますか？", "a": "基本の選び方は同じですが、金属スパイクは硬い土・人工芝で衝撃が伝わりやすい傾向があるため、衝撃吸収性を重視すると安心です。いずれも元の中敷きと厚みを揃えて入れ替え、フィットを確かめましょう。"}, {"q": "サイズはどう選べばよいですか？", "a": "多くのインソールはS・M・Lなどの幅で展開され、つま先側をハサミでカットして微調整できる製品もあります。今のスパイクの中敷きと重ねて長さ・幅を比べ、迷ったら大きめを選んで切り詰めるのが安全です。具体的な対応サイズは各商品の公式サイト・店頭表示で確認してください。"}, {"q": "成長期の子ども（少年野球）にも使えますか？", "a": "足のサイズが変わりやすい時期なので、買い替え前提でフィットを優先しましょう。足や成長に不安がある場合は、無理をせず専門家や医療機関に相談することをおすすめします。"}];
  ...
  return (
    <>
      ...
      <article className="max-w-3xl mx-auto px-4 py-8 md:py-12">
        ...
        <section className="mb-12">
          <h2 className="text-lg md:text-2xl font-bold text-gray-900 border-l-4 border-[#0ea5e9] pl-3 mb-6">素材とサイズの選び方</h2>
          <p className="text-sm md:text-base text-gray-700 leading-relaxed mb-4">インソールの素材は、足裏を支えるアーチ部分の硬さと、衝撃を受け止めるクッション部分の柔らかさのバランスで選びます。一般に、硬めの素材はアーチをしっかり支えて踏み込みの力が逃げにくく、柔らかめの素材はクッション性に優れて着地の衝撃をやわらげます。野球では踏ん張りとダッシュ・ストップの衝撃の両方があるため、アーチは支えつつ、かかと側にクッションのある構造が扱いやすい傾向です。</p>
          <p className="text-sm md:text-base text-gray-700 leading-relaxed mb-4">サイズは、今使っているスパイクの中敷きと重ねて長さ・幅を比べるのが確実です。多くの製品はS・M・Lなどの幅で展開され、つま先側をカットして微調整できるタイプもあります。迷ったときは大きめを選んで切り詰めると失敗しにくいでしょう。汗をかきやすい競技なので、通気性や抗菌・防臭への配慮があるかも確認しておくと快適です。具体的な対応サイズ・素材の仕様や価格は、各商品の公式サイトや店頭表示で確認してください。</p>
        </section>
        ...
        <section className="mb-12">
          <h2 className="text-lg md:text-2xl font-bold text-gray-900 border-l-4 border-[#0ea5e9] pl-3 mb-6">使用上の注意点</h2>
          <p className="text-sm md:text-base text-gray-700 leading-relaxed mb-4">新しいインソールに替えた直後は、足裏やアーチの当たり方が変わって違和感を覚えることがあります。いきなり試合で使うのではなく、まず練習で短時間から慣らし、フィットを確かめてから本番に投入すると安心です。痛みやしびれが続く場合は使用を中止しましょう。</p>
          <p className="text-sm md:text-base text-gray-700 leading-relaxed mb-4">インソールは消耗品で、使い続けるとクッションやアーチのへたりが進みます。へたってきたら早めに交換し、左右のすり減り方に偏りがないかも時々チェックしましょう。グラウンドの砂や汗で汚れやすいため、使用後は取り出して陰干しし、清潔に保つと長持ちします。</p>
          <p className="text-sm md:text-base text-gray-700 leading-relaxed mb-4">なお、インソールの効き方には個人差があり、履くだけでパフォーマンスが上がったり痩せたりするわけではありません。足の痛みや変形など気になる症状がある場合は、自己判断で対処せず、専門家や医療機関に相談することをおすすめします。</p>
        </section>

...
const faqs = [{"q": "Amazonと楽天どっちが安いですか？", "a": "タイミングによります。楽天はポイント還元、Amazonはセールやクーポンでお得になることがあります。ポイントを含めた実質価格で比較しましょう。"}, {"q": "ネットで偽物を避けるには？", "a": "メーカー公式ストアや正規販売店から購入することです。販売元・正規品保証・返金保証を確認し、極端に安い非正規出品は避けましょう。"}, {"q": "公式サイトとAmazon・楽天はどちらが良いですか？", "a": "返金保証や最安値を重視するなら公式サイト、ポイントや配送の手軽さなら大手EC。解約条件や保証内容を比較して選びましょう。"}, {"q": "ECで『公式ストア』かどうかはどこで見分けますか？", "a": "商品ページの『販売元』『出荷元』の表記を確認します。メーカー名や正規代理店名になっているか、ストア名やレビュー欄で正規取扱の記載があるかをチェックしましょう。不明な場合はメーカー公式サイトの取扱店情報と照らし合わせると確実です。"}, {"q": "サイズはどう選べばいいですか？", "a": "多くのインソールは普段の靴のサイズ表記に対応していますが、対応サイズの範囲やカット調整の可否は商品ごとに異なります。商品ページのサイズ表と、必要に応じてカットして使うタイプかを購入前に確認しましょう。並行輸入品は表記が海外基準の場合があるので注意が必要です。"}];
    ...
    <>
      ...
      <article className="max-w-3xl mx-auto px-4 py-8 md:py-12">
        ...
        <section className="mb-12">
          ...
          <p className="text-sm md:text-base text-gray-700 leading-relaxed mb-4">同じ商品名でも、メーカー公式ストアからの出品と、第三者が出品する『並行輸入品』や転売品が並ぶことがあります。並行輸入品は正規ルート外のため、メーカーの返金保証・正規品保証の対象外になる場合や、サイズ表記が海外基準で異なる場合があります。購入前に商品ページの『販売元』『出荷元』の表記を確認し、メーカー名や正規代理店名になっているかをチェックしましょう。</p>
        </section>
        ...
        <section className="mb-12">
          ...
          <p className="text-sm md:text-base text-gray-700 leading-relaxed mb-4">レビューを参考にする際は、件数の多さと内容の具体性を見るのがポイントです。総合評価の星の数だけでなく、サイズ感や履き心地に触れた具体的なレビューが複数あるかを確認しましょう。極端な高評価・低評価が短期間に集中している場合は、参考程度にとどめるのが無難です。サイズは商品ごとに対応範囲やカット調整の可否が異なるため、商品ページのサイズ表を必ず確認してください。</p>
        </section>

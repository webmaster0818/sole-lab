...
const faqs = [{"q": "インソールで体幹は鍛えられますか？", "a": "単体で鍛えるものではありませんが、足元を安定させて正しい姿勢を促し、日常で体幹を使いやすくする補助になります。トレーニングとの併用が効果的です。"}, {"q": "バランス感覚は良くなりますか？", "a": "足元が安定すると姿勢のブレが減り、バランスを保ちやすくなります。高齢の方の転倒予防の観点でも足元の安定は重要です。"}, {"q": "体幹トレーニングと併用すべきですか？", "a": "はい。インソールで日常の姿勢を整えつつ、プランクなどの体幹トレーニングを行うと相乗効果が期待できます。"}, {"q": "インソールだけで体幹トレーニングの代わりになりますか？", "a": "なりません。体幹の筋肉を鍛えるには負荷をかけた運動が必要で、インソールはあくまで姿勢を整え体幹を意識しやすくする補助です。トレーニングの置き換えではなく、日常での意識づけとして併用する位置づけです。"}, {"q": "インソールはどう選べばよいですか？", "a": "足裏のアーチを支える設計で、普段の靴に入り長時間履いても疲れにくいものが基本です。実際に立ったり歩いたりした際の安定感やフィット感を確かめ、自分の足の形・靴に合うかを基準に選ぶとよいでしょう。"}];
    ...
    <>
      ...
      <article className="max-w-3xl mx-auto px-4 py-8 md:py-12">
        ...
        <section className="mb-12">
          ...
          <p className="text-sm md:text-base text-gray-700 leading-relaxed mb-4">人の体は足裏で地面と接しており、足元のわずかな傾きやぐらつきは膝・骨盤・背骨へと連なって全身の姿勢に影響します。足裏のアーチが崩れると重心が偏り、本来体幹で支えるべき負担を腰や脚の特定の筋肉が肩代わりしやすくなります。インソールでアーチを補助することは、こうした重心の偏りをならし、体幹で姿勢を支える感覚を取り戻す土台づくりにつながると考えられます。</p>
          <p className="text-sm md:text-base text-gray-700 leading-relaxed">ただしインソールはあくまで足元の環境を整える道具であり、それ自体が筋肉を鍛えるものではありません。効果の感じ方には個人差があり、姿勢や歩き方の癖、靴との相性によっても変わります。過度な期待はせず、日常の意識づけを助ける補助として取り入れるのが現実的です。</p>
        </section>
